...
export class SessionModel {
    ...
    async show(id: number): Promise<Session> {
      ...
      async create(se: Session ): Promise<Session> {
        try {
          const connection = await client.connect();
          const sql =
            'INSERT INTO sessions (date, title, sl_id) VALUES($1, $2, $3) RETURNING *';
          const result = await connection.query(sql, [se.date, se.title, se.sl_id]);
          connection.release();
          return result.rows[0];
        } catch (error) {
          throw new Error(
      ...
      async addStudent(studentId: number, sessionId: number): Promise<Session> {
        try {
